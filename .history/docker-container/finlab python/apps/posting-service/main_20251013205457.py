...
@app.get("/posts")
async def get_all_posts(skip: int = 0, limit: int = 2000, status: Optional[str] = None):
    """獲取所有貼文"""
    try:
    ...
    try:
        ...
        for post in all_posts:
            kol_info = {
                ...
                "member_id": f"950{post.kol_serial:04d}"  # 生成 member_id
            }
            kol_set.add((kol_info["serial"], kol_info["nickname"], kol_info["persona"], kol_info["member_id"]))
        
        kols = []
        for serial, nickname, persona, member_id in kol_set:
            kols.append({
                "serial": serial,
                "nickname": nickname,
                "persona": persona,
...
@app.get("/api/manual-posting/kols")
async def get_manual_posting_kols():
    """獲取手動發文可用的 KOL 列表"""
    try:
        all_posts = get_post_record_service().get_all_posts()
        ...
        for post in all_posts:
            ...
            kol_set.add((kol_info["serial"], kol_info["nickname"], kol_info["persona"]))
        
        kols = []
        for serial, nickname, persona in kol_set:
            kols.append({
                "serial": serial,
                "nickname": nickname,
                "persona": persona
...
@app.get("/api/manual-posting/stocks")
async def get_manual_posting_stocks():
    """獲取手動發文可用的股票列表"""
    try:
        all_posts = get_post_record_service().get_all_posts()
        ...
        for post in all_posts:
            stock_info = {
                "code": post.stock_code,
                "name": post.stock_name
            }
            stock_set.add((stock_info["code"], stock_info["name"]))
        
        stocks = []
        for code, name in stock_set:
            stocks.append({
                "code": code,
                "name": name
            })
...
@app.get("/api/manual-posting/trending-topics")
async def get_manual_posting_topics():
    """獲取手動發文可用的熱門話題"""
    try:
        all_posts = get_post_record_service().get_all_posts()
        ...
        for post in all_posts:
            if hasattr(post, 'commodity_tags') and post.commodity_tags:
                tags = post.commodity_tags if isinstance(post.commodity_tags, list) else [post.commodity_tags]
                for tag in tags:
                    topic_set.add(tag)
        
        topics = list(topic_set) if topic_set else [
            "技術分析", "基本面分析", "市場趨勢", "個股研究", "投資策略"
        ]
        ...
        logger.error(f"獲取話題列表失敗: {e}")
        raise HTTPException(status_code=500, detail=f"獲取話題列表失敗: {str(e)}")

@app.get("/api/manual-posting/recent-posts")
async def get_manual_posting_recent_posts(limit: int = 10):
    """獲取手動發文的最近記錄"""
    try:
        all_posts = get_post_record_service().get_all_posts()
        ...
        logger.error(f"獲取最近發文失敗: {e}")
        raise HTTPException(status_code=500, detail=f"獲取最近發文失敗: {str(e)}")

@app.get("/api/manual-posting/batch-history")
async def get_manual_posting_batch_history(limit: int = 20):
    """獲取手動發文的批次歷史"""
    try:
        all_posts = get_post_record_service().get_all_posts()
        ...
        session_groups = {}
        for post in all_posts:
            session_id = post.session_id or "manual"
            if session_id not in session_groups:
                session_groups[session_id] = []
            session_groups[session_id].append(post)
        ...
        for session_id, posts in session_groups.items():
            ...
            # 推斷觸發類型
            trigger_type = "manual" if session_id == "manual" else "batch"
            
            # 提取 KOL 和股票資訊
            kol_names = list(set([p.kol_nickname for p in posts]))
            stock_codes = list(set([p.stock_code for p in posts]))
            
            # 使用最早的創建時間
            created_at = min([p.created_at for p in posts if p.created_at])
            
            batch_history.append({
                "session_id": str(session_id),
                "created_at": created_at.isoformat() if created_at else None,
                "status": "completed" if published_posts > 0 else "pending",
                "trigger_type": trigger_type,
                "kol_assignment": f"{len(kol_names)} KOLs",
        ...
        # 按創建時間排序，取最新的
        batch_history.sort(key=lambda x: x["created_at"] or "", reverse=True)
        ...
        logger.error(f"獲取批次歷史失敗: {e}")
        raise HTTPException(status_code=500, detail=f"獲取批次歷史失敗: {str(e)}")

@app.post("/api/manual-posting/submit")
async def submit_manual_post(request: dict):
    """提交手動發文"""
    try:
        # 生成 post_id
        post_id = f"manual_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{request.get('kol_serial', 'unknown')}"
        
        # 創建 PostRecordInDB 對象
        post_record = PostRecordInDB(
            post_id=post_id,
            session_id=None,  # 手動發文沒有 session_id
            kol_serial=request.get('kol_serial'),
            kol_nickname=request.get('kol_nickname'),
            kol_persona=request.get('kol_persona'),
            stock_code=request.get('stock_code'),
            stock_name=request.get('stock_name'),
            title=request.get('title'),
            content=request.get('content'),
            content_md=request.get('content_md'),
            status='draft',  # 手動發文預設為草稿
            commodity_tags=request.get('commodity_tags', []),
            generation_params=request.get('generation_params', {}),
            quality_score=request.get('quality_score'),
            ai_detection_score=request.get('ai_detection_score'),
        ...
        # 保存到資料庫
        result = get_post_record_service().save_to_postgresql(post_record)
        ...
        if result:
            ...
            raise HTTPException(status_code=500, detail="保存手動發文失敗")
            
    except Exception as e:
        logger.error(f"提交手動發文失敗: {e}")
        raise HTTPException(status_code=500, detail=f"提交手動發文失敗: {str(e)}")

@app.get("/api/manual-posting/health")
async def manual_posting_health():
    """手動發文服務健康檢查"""
...
async def get_schedule_tasks():
    """獲取排程任務列表"""
    try:
        # 從 post_records 中查找有 scheduled_at 的貼文
        all_posts = get_post_record_service().get_all_posts()
        
        scheduled_posts = [post for post in all_posts if post.scheduled_at]
        ...
        # 按排程時間排序
        tasks.sort(key=lambda x: x["scheduled_at"] or "", reverse=True)
...
async def get_schedule_daily_stats():
    """獲取排程每日統計"""
    try:
        all_posts = get_post_record_service().get_all_posts()
        ...
        for post in all_posts:
            if post.created_at:
                ...
                daily_stats[date_key]["total_posts"] += 1
                
                if post.scheduled_at:
                    daily_stats[date_key]["scheduled_posts"] += 1
                
                if post.status == "published":
                    daily_stats[date_key]["published_posts"] += 1
                elif post.status in ["draft", "pending-review"]:
                    daily_stats[date_key]["pending_posts"] += 1
...
async def get_kol_profiles():
    ...
    try:
        ...
        for post in all_posts:
            kol_key = f"{post.kol_nickname}_{post.kol_persona}"
            if kol_key not in kol_set:
                kol_set.add(kol_key)
                kol_profiles.append({
                    "id": len(kol_profiles) + 1,
                    "serial": f"KOL{len(kol_profiles) + 1:03d}",
                    "nickname": post.kol_nickname,
                    "member_id": f"M{len(kol_profiles) + 1:03d}",
                    "persona": post.kol_persona,
                    "status": "active",
                    "owner": "system",
                    "email": f"kol{len(kol_profiles) + 1}@example.com",
                    "password": "******",
                    "target_audience": "投資者",
                    "notes": f"{post.kol_persona} KOL",
                    "prompt_persona": f"你是一個{post.kol_persona}的投資分析師",
                    "prompt_style": "專業且易懂",
                    "prompt_guardrails": "遵守投資建議規範",
                    "prompt_skeleton": "分析 + 結論 + 建議",
                    "model_id": "gpt-4",
                    "model_temp": 0.7,
                    "max_tokens": 1000,
                    "template_variant": "standard",
                    "signature": f"- {post.kol_nickname}",
                    "emoji_pack": "finance",
                    "tone_formal": 7,
                    "tone_emotion": 5,
                    "tone_confidence": 8,
                    ...
                    "interaction_starters": ["你怎麼看？", "歡迎討論"],
                    "require_finlab_api": True,
                    "allow_hashtags": True,
                    "created_time": "2024-01-01T00:00:00Z",
                    "last_updated": "2024-10-13T00:00:00Z",
                    "total_posts": len([p for p in all_posts if p.kol_nickname == post.kol_nickname]),
                    "published_posts": len([p for p in all_posts if p.kol_nickname == post.kol_nickname and p.status == 'published']),
                    "avg_interaction_rate": 0.15,
                    "best_performing_post": "技術分析貼文",
        ...
        logger.error(f"獲取 KOL 設定檔失敗: {e}")
        raise HTTPException(status_code=500, detail=f"獲取 KOL 設定檔失敗: {str(e)}")

@app.put("/kol-profiles/{serial}/personalization")
async def update_kol_personalization(serial: str, personalization_data: dict):
    """更新 KOL 個人化設定"""
    try:
        # 這裡可以實現更新 KOL 設定的邏輯
        # 目前先返回成功
        return {
            "success": True,
            "message": f"KOL {serial} 個人化設定已更新",
            "data": personalization_data
        }
    except Exception as e:
        logger.error(f"更新 KOL 個人化設定失敗: {e}")
        raise HTTPException(status_code=500, detail=f"更新 KOL 個人化設定失敗: {str(e)}")


# ==================== 盤中觸發器 API 端點 ====================

@app.post("/api/intraday-trigger/limit-up")
async def trigger_intraday_limit_up(request: dict):
    """盤中漲停觸發器"""
    try:
        stock_code = request.get('stock_code')
        stock_name = request.get('stock_name')
        trigger_time = request.get('trigger_time', datetime.now().isoformat())
        ...
        # 這裡可以添加自動發文邏輯
        # 例如：自動選擇合適的 KOL 和生成內容
        
        return {
            "success": True,
            "message": f"盤中漲停觸發器已啟動: {stock_name}({stock_code})",
            "data": trigger_event
        }
    except Exception as e:
        logger.error(f"盤中漲停觸發器失敗: {e}")
        raise HTTPException(status_code=500, detail=f"盤中漲停觸發器失敗: {str(e)}")

@app.post("/api/intraday-trigger/limit-down")
async def trigger_intraday_limit_down(request: dict):
    """盤中跌停觸發器"""
    try:
        stock_code = request.get('stock_code')
        stock_name = request.get('stock_name')
        ...
        # 記錄觸發事件
        trigger_event = {
            "trigger_type": "intraday_limit_down",
            "stock_code": stock_code,
            "stock_name": stock_name,
            "trigger_time": trigger_time,
            "status": "triggered",
        ...
        return {
            "success": True,
            "message": f"盤中跌停觸發器已啟動: {stock_name}({stock_code})",
            "data": trigger_event
        }
    except Exception as e:
        logger.error(f"盤中跌停觸發器失敗: {e}")
        raise HTTPException(status_code=500, detail=f"盤中跌停觸發器失敗: {str(e)}")

@app.get("/api/intraday-trigger/history")
async def get_intraday_trigger_history(limit: int = 50):
    """獲取盤中觸發器歷史"""
    try:
        # 這裡可以從數據庫獲取觸發器歷史
        # 目前返回模擬數據
        trigger_history = [
            {
                ...
                "stock_code": "2330",
                "stock_name": "台積電",
                "trigger_time": "2024-10-13T09:30:00Z",
                "status": "completed",
                "created_at": "2024-10-13T09:30:00Z"
            },
            {
                "id": 2,
                "trigger_type": "intraday_limit_down",
                "stock_code": "2317",
                "stock_name": "鴻海",
                "trigger_time": "2024-10-13T10:15:00Z",
                "status": "completed",
        ...
        return {
            "success": True,
            "data": trigger_history[:limit],
            "total": len(trigger_history)
        }
    except Exception as e:
        logger.error(f"獲取盤中觸發器歷史失敗: {e}")
        raise HTTPException(status_code=500, detail=f"獲取盤中觸發器歷史失敗: {str(e)}")

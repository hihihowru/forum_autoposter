...
const ScheduleManagementPage: React.FC = () => {
  ...
  // 計算倒計時
  const getCountdown = (nextRun: string | null, scheduleType: string = '', scheduleConfig: any = null) => {
    if (!nextRun) return '';
  ...
  const loadDailyStats = async () => {
    try {
      const response = await fetch('/api/schedule/daily-stats');
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const result = await response.json();
          ...
          {nextRun ? (
            <>
              ...
              <div>
                ...
                >
                  {getCountdown(nextRun, config.schedule_type, config)}
                </Text>
              </div>
            </>
          ) : (
          ...
          <Card>
            ...
            {statistics.nextExecution && (
              <Text type="secondary" style={{ fontSize: 12 }}>
                最近: {getCountdown(statistics.nextExecution, statistics.nextScheduleType, null)}
              </Text>
            )}
          </Card>
        </Col>

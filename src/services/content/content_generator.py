...
class ContentGenerator:
    ...
    def _format_market_data(self, market_data: Optional[Dict[str, Any]]) -> str:
        ...
        limit_up_info = f"\n漲停資訊:\n- 漲幅: {change_percent:.2f}%\n- 現價: {current_price}\n- 成交量: {volume:,} 張"
        
        # 營收數據
        revenue_data = market_data.get('revenue_data', {})
        revenue_info = ""
        if revenue_data and revenue_data.get('success', False):
            revenue_detail = revenue_data.get('data', {})
            if revenue_detail:
                current_revenue = revenue_detail.get('current_revenue', {})
                growth = revenue_detail.get('growth', {})
                revenue_info = f"\n營收數據:\n- 當月營收: {current_revenue.get('formatted', 'N/A')}\n- 年增率: {growth.get('year_over_year', 0):.2f}%"
        
        # 財報數據
        financial_data = market_data.get('financial_data', {})
        financial_info = ""
        if financial_data and financial_data.get('success', False):
            financial_detail = financial_data.get('data', {})
            if financial_detail:
                profitability = financial_detail.get('profitability', {})
                financial_info = f"\n財報數據:\n- 營業利益率: {profitability.get('operating_margin', 0):.2f}%\n- ROE: {profitability.get('roe', 0):.2f}%"
        
        # 技術數據
        technical_data = market_data.get('technical_data', {})
        technical_info = ""
        if technical_data and technical_data.get('success', False):
            technical_detail = technical_data.get('data', {})
            if technical_detail:
                technical_info = f"\n技術分析:\n- 技術評分: {technical_detail.get('score', 'N/A')}\n- 趨勢: {technical_detail.get('trend', 'N/A')}"
        
        # 新聞來源
        news_sources = market_data.get('news_sources', [])
        news_info = ""
        if news_sources:
            news_info = f"\n相關新聞: {len(news_sources)} 則"
        
        # 分析來源
        analysis_sources = market_data.get('analysis_sources', [])
        analysis_info = ""
        if analysis_sources:
            analysis_info = f"\n分析報告: {len(analysis_sources)} 份"
        
        return f"{basic_info}{limit_up_info}{revenue_info}{financial_info}{technical_info}{news_info}{analysis_info}"

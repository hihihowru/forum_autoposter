...
interface TriggerConfig {
  triggerType: 'individual' | 'sector' | 'macro' | 'news' | 'intraday';
  triggerKey: string;
  stockFilter: string;
  volumeFilter?: string;
  sectorFilter?: string;
...
const TriggerSelector: React.FC<TriggerSelectorProps> = ({ value, onChange }) => {
  ...
  // 盤中觸發器股票查詢
  const handleIntradayStockQuery = async () => {
    if (!value.triggerConfig?.apiConfig) {
      message.warning('請先選擇盤中觸發器');
      return;
    }
    ...
    try {
      // 構建查詢參數
      const queryParams = {
        triggerKey: value.triggerConfig.triggerKey,
        apiConfig: value.triggerConfig.apiConfig,
        stockCountLimit: value.stockCountLimit || 10,
        threshold: value.threshold || 1000
      };

      console.log('盤中觸發器查詢參數:', queryParams);

      // 調用盤中觸發器 API
      const response = await fetch('http://localhost:8001/api/intraday-trigger/query', {
        method: 'POST',
        headers: {
      ...
      if (result.success && result.stocks && result.stocks.length > 0) {
        // 自動選取查詢到的股票
        const stockCodes = result.stocks.map((stock: any) => stock.stock_code || stock.code);
        const stockNames = result.stocks.map((stock: any) => stock.stock_name || stock.name || `股票${stock.stock_code || stock.code}`);
        
        // 限制選取數量
        const limitedCodes = stockCodes.slice(0, value.stockCountLimit || 10);
        const limitedNames = stockNames.slice(0, value.stockCountLimit || 10);
        
        onChange({
          ...value,
          stock_codes: limitedCodes,
          stock_names: limitedNames
        });
        
        message.success(`已自動選取 ${limitedCodes.length} 支盤中股票`);
        
        // 更新公司名稱映射
        const mapping: Record<string, string> = {};
        result.stocks.forEach((stock: any) => {
          if (stock.stock_code || stock.code) {
            mapping[stock.stock_code || stock.code] = stock.stock_name || stock.name || `股票${stock.stock_code || stock.code}`;
          }
        });
        setCompanyNameMapping(mapping);
        
      } else {
        message.warning('未找到符合條件的盤中股票');
      }
    } catch (error) {
      console.error('盤中觸發器查詢失敗:', error);
      
      // 如果 API 不存在，使用模擬數據
      if (error instanceof Error && error.message.includes('404')) {
        message.info('盤中觸發器 API 尚未實現，使用模擬數據');
        
        // 生成模擬股票數據
        const mockStocks = generateMockIntradayStocks(value.triggerConfig.triggerKey, value.stockCountLimit || 10);
        
        const stockCodes = mockStocks.map(stock => stock.code);
        const stockNames = mockStocks.map(stock => stock.name);
        ...
        message.success(`已選取 ${stockCodes.length} 支模擬盤中股票`);
      } else {
        message.error('盤中觸發器查詢失敗，請稍後再試');
      }
    } finally {
      setStockCountLoading(false);
    }
  ...
  // 生成模擬盤中股票數據
  const generateMockIntradayStocks = (triggerKey: string, count: number) => {
    const baseStocks = [
      { code: '2330', name: '台積電' },
      { code: '2454', name: '聯發科' },
      { code: '2317', name: '鴻海' },
    ...
    // 根據觸發器類型選擇不同的股票
    let selectedStocks = baseStocks;
    
    switch (triggerKey) {
      case 'intraday_gainers_by_amount':
        selectedStocks = baseStocks.slice(0, count); // 漲幅排序
        break;
      case 'intraday_volume_leaders':
        selectedStocks = baseStocks.slice(2, 2 + count); // 成交量排序
        break;
      case 'intraday_amount_leaders':
        selectedStocks = baseStocks.slice(4, 4 + count); // 成交額排序
        break;
      case 'intraday_limit_up':
        selectedStocks = baseStocks.slice(0, Math.min(count, 5)); // 漲停
        break;
      case 'intraday_limit_down':
        selectedStocks = baseStocks.slice(10, 10 + count); // 跌停
        break;
      case 'intraday_limit_down_by_amount':
        selectedStocks = baseStocks.slice(15, 15 + count); // 跌停+成交額
        break;
      default:
        selectedStocks = baseStocks.slice(0, count);
    }

    return selectedStocks.slice(0, count);
  };
  ...
  return (
    <div>
      ...
      {value.triggerConfig?.triggerType === 'intraday' && value.triggerConfig?.apiConfig && (
        <Card title="盤中觸發器股票查詢" size="small" style={{ marginTop: 16 }}>
          <Row gutter={16} align="middle">
            <Col span={12}>
              <Space direction="vertical" style={{ width: '100%' }}>
                <Text strong>已選擇觸發器：{value.triggerConfig.triggerKey}</Text>
                <Text type="secondary">
                  {value.triggerConfig.apiConfig.endpoint}
                </Text>
              </Space>
            </Col>
            <Col span={12}>
              <Space>
                <Button 
                  type="primary" 
                  icon={<SearchOutlined />}
                  onClick={handleIntradayStockQuery}
                  loading={stockCountLoading}
                  size="large"
                >
                  查詢並選取盤中股票
          ...
          <Row gutter={16} style={{ marginTop: 16 }}>
            ...
            <Col span={8}>
              <Space>
                <Text strong>已選股票：</Text>
                <Tag color="blue">{value.stock_codes?.length || 0} 支</Tag>
              </Space>
            </Col>
            <Col span={8}>
              <Space>

...
class CMoneyClient:
    ...
    async def get_article_interactions(self, access_token: str, article_id: str) -> InteractionData:
        ...
        try:
            ...
            total_emojis = sum(emoji_count.values()) if isinstance(emoji_count, dict) else 0
            
            # 計算互動率 (簡單版本)
            comments = result.get("commentCount", 0)
            interests = result.get("interestedCount", 0)  # CMoney 的興趣數相當於讚數
            collections = result.get("collectedCount", 0)
            
            total_interactions = comments + interests + collections + total_emojis
            # 注意：CMoney API 沒有直接提供瀏覽數，這裡暫時用互動數
            engagement_rate = total_interactions  # 可以後續根據實際需求調整計算方式
            
            # 建立互動數據物件
            interaction = InteractionData(
                post_id=article_id,
                member_id=str(result.get("creatorId", "")),
                likes=interests,  # CMoney 的 interestedCount 對應讚數
                comments=comments,
            ...
            logger.info(f"成功獲取文章 {article_id} 的互動數據: 讚={interests}, 留言={comments}, 收藏={collections}, 表情={total_emojis}")
            return interaction

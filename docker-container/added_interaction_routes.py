...
async def get_kol_credentials(kol_serial: int) -> Optional[LoginCredentials]:
    ...
    try:
        # KOL 憑證映射表（使用正確的密碼）
        kol_credentials_map = {
            200: LoginCredentials(email='forum_200@cmoney.com.tw', password='N9t1kY3x'),  # 川川哥
            201: LoginCredentials(email='forum_201@cmoney.com.tw', password='m7C1lR4t'),  # KOL-201
            202: LoginCredentials(email='forum_202@cmoney.com.tw', password='N9t1kY3x'),  # KOL-202
            203: LoginCredentials(email='forum_203@cmoney.com.tw', password='N9t1kY3x'),  # KOL-203
            204: LoginCredentials(email='forum_204@cmoney.com.tw', password='N9t1kY3x'),  # KOL-204
            205: LoginCredentials(email='forum_205@cmoney.com.tw', password='N9t1kY3x'),  # KOL-205
            206: LoginCredentials(email='forum_206@cmoney.com.tw', password='N9t1kY3x'),  # KOL-206
            207: LoginCredentials(email='forum_207@cmoney.com.tw', password='N9t1kY3x'),  # KOL-207
            208: LoginCredentials(email='forum_208@cmoney.com.tw', password='N9t1kY3x'),  # KOL-208
        }
        
        credentials = kol_credentials_map.get(kol_serial)
        if credentials:
            logger.info(f"✅ 找到 KOL {kol_serial} 的憑證")
            return credentials
        else:
            logger.warning(f"⚠️ 找不到 KOL {kol_serial} 的憑證，使用預設憑證")
            # 回退到川川哥的憑證
            return LoginCredentials(email='forum_200@cmoney.com.tw', password='N9t1kY3x')
...
async def get_cmoney_interaction_data(article_id: str, kol_serial: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not credentials:
            logger.error(f"❌ 無法從數據庫獲取 KOL {kol_serial} 的憑證")
            return None

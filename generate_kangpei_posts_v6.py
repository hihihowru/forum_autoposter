...
請直接生成標題和內容，不需要額外說明。"""
    ...
    def _parse_llm_response(self, response: str) -> Dict[str, Any]:
        """解析 LLM 回應 - 修正版本，移除前綴"""
        try:
            # 清理回應內容，移除 "標題：" 和 "內容：" 前綴
            cleaned_response = response.replace("標題：", "").replace("內容：", "").strip()
            ...
            # 其餘行作為內容
            content_lines = [line.strip() for line in lines if line.strip() and line.strip() != title]
            content = '\n'.join(content_lines)
            
            # 如果沒有找到明確的內容，將整個回應作為內容
            if not content:
                content = cleaned_response
            
            # 清理標題中的多餘符號
            title = title.strip()
            if title.startswith("標題"):
                title = title[2:].strip()
...
class KangpeiPostGeneratorV6:
    ...
    async def generate_kol_post(self, kol: Dict[str, Any], news_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            post_data = {
                ...
                'kol_settings_version': 'v6.0_enhanced_with_personalization',  # AD: KOL設定版本
    ...
    async def update_posts_sheet(self, posts: List[Dict[str, Any]]):
        ...
        try:
            ...
            # 使用 append_sheet 方法寫入數據
            result = self.sheets_client.append_sheet('貼文記錄表', new_rows)
            logger.info(f"✅ 成功寫入 {len(posts)} 筆記錄到 Google Sheets")
    ...
    async def run(self):
        """執行貼文生成流程"""
        logger.info("🚀 開始生成 6919 康霈生技相關貼文 (V6)...")
...
async def main():
    """主函數"""
    generator = KangpeiPostGeneratorV6()
    await generator.run()

...
const BatchHistoryPage: React.FC = () => {
  ...
  const handleConfirmSchedule = async (scheduleConfig: any) => {
    try {
      ...
      // 創建排程
      const response = await fetch('http://localhost:8001/schedule/create', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        // 更新貼文的排程狀態
        const updateResponse = await fetch('http://localhost:8001/schedule/update-posts', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (updateResult.success) {
          ...
          message.error('更新貼文排程狀態失敗: ' + updateResult.message);
        }
      } else {

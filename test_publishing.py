...
async def main():
    ...
    try:
        ...
        kol_credentials = [
            (200, 'forum_200@cmoney.com.tw', 'N9t1kY3x'),
            (201, 'forum_201@cmoney.com.tw', 'm7C1lR4t'),
            (202, 'forum_202@cmoney.com.tw', 'x2U9nW5p')
        ]
        
        success_count = 0
        for serial, email, password in kol_credentials:
            print(f"登入 KOL {serial}...")
            success = await publish_service.login_kol(serial, email, password)
            if success:
                success_count += 1
                print(f"✅ KOL {serial} 登入成功")
            else:
                print(f"❌ KOL {serial} 登入失敗")
        
        if success_count == 0:
            print("❌ 沒有 KOL 登入成功，無法繼續發文")
            return
        
        print(f"✅ {success_count}/{len(kol_credentials)} 個 KOL 登入成功")
